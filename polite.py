...
def sheng(f0,count):
    n=len(f0)
    max_uper,temp_uper=0,0
    for i in range(n-count+1,n-1):
        if f0[i]-f0[i-1]>=0.1:
            temp_uper+=f0[i]-f0[i-1]
            max_uper=max(max_uper,temp_uper)
        elif f0[i]-f0[i-1]<=0.1 and f0[i]-f0[i-1]>=-0.3:
            continue
        elif f0[i]-f0[i-1]<0.3:
            max_uper=max(max_uper,temp_uper)
            temp_uper=0
    return max_uper


def findcount():        #寻找最后两个词的音素个数
