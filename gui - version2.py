...
def getlist(filename):
    ...
    voicetime=[]
    h12txtfile = open("data/segmentresult/"+filename+'.bid','r', encoding="utf-8")
    lines = h12txtfile.readlines()
    for line in lines:    #temp_list中[1]代表h1-h2，[2]代表h1*-h2*，
        temp_list=line.split()
        if temp_list[1]!="h1-h2" and len(temp_list[0])>=3:
            h12.append(float(temp_list[1]))
    h12_mean=mean(h12)
    h12_cha=max(h12)-min(h12)
    f0txtfile = open("data/segmentresult/"+filename+'.actutimesemitonef0','r', encoding="utf-8")
    ...
    保存在voicespeed中
    """
    voicetimetext=open("data/segmentresult/"+'allData.txt','r', encoding="utf-8")
    lines=voicetimetext.readlines()
    for line in lines:
        temp_list=line.split()
        if temp_list[0]==filename:
            voicetime.append(float(temp_list[4])-float(temp_list[3]))
    voicespeed=mean(voicetime)
    return upertime,voicespeed,h12_mean, h12_cha,f0_mean
"""
computepolite函数通过参数计算礼貌度
...
def textcomputepolite(text):
    politescore=0
    politeword=["please","thank"]
    tense_begin=["was","were"]
    sentence_flag=["if","could"]
    #礼貌词评分
    ...
    sentence_score=0
    for i in range(len(word_list)):
        if word_list[i] in sentence_flag:
            sentence_score+=5
...
def show():
    ...
    temp_text=open('save.txt','r', encoding="utf-8")
    ...
    lable4 = tk.Label(root,text = '                         ')
    lable5 = tk.Label(root,text = '                         ')
    lable6 = tk.Label(root,text = '                         ')
    lable1 = tk.Label(root,text = "最后升调持续时间："+str(upertime))
    lable2 = tk.Label(root,text = "语速："+str(voicespeed))
    lable3 = tk.Label(root,text = "谐波差均值："+str(h12_mean))
    lable4 = tk.Label(root,text = '基频均值：'+str(f0_mean))
    lable5 = tk.Label(root,text = '语音礼貌度分数：'+str(score))
    lable6 = tk.Label(root,text = '语义礼貌度分数：'+str(word_score+tense_score+sentence_score))
    lable7 = tk.Label(root,text = "                ")
...
def luyin():        #调用麦克风进行录音并转换为英文
    print("ok")
    translate.luyin()
    entext=translate.googletranslatevoice("output10.wav")
    with open("save.txt","w",encoding="utf-8") as t:
        t.write(str(entext))
    t.close()
    print("voice to word sucessfully")
    chtext=translate.tochinse(entext)
    lable7 = tk.Label(root,text = "        ")
    lable7 = tk.Label(root,text = "语句："+str(entext))
    lable7.place(x=100,y=200)
...
button1 = tk.Button(root, text="选择文件", font=("Monospaced", 10),height=3,width=10,command=show,bg="Ivory")
